...
public class NetController {
	...
	private OutgoingSock[] outSockets;
	...
	public NetController(Config config, BlockingQueue<Message> leaderQueue,
	...
	public NetController(Config config, int numOfServers,
			BlockingQueue<Message> clientQueue) {
		this.clientQueue = clientQueue;
		this.config = config;
		this.numOfServers = numOfServers;
		inSockets = new IncomingSock[config.numProcesses];
		listener = new ListenServer(config, inSockets, leaderQueue, null, null,
				null, null, null, clientQueue);
		outSockets = new OutgoingSock[config.numProcesses];
		listener.start();
	}
	...
	private synchronized void initOutgoingConn(int proc) throws IOException {
		...
		Socket bareSocket = new Socket(config.addresses[proc],
				config.ports[proc]);
		ObjectOutputStream outputStream = new ObjectOutputStream(
				bareSocket.getOutputStream());
		// Send your process ID to the server to which you just initiated the
		// connection.
	 ...
	 * @param msg
	 *            Do not use the "&" character. This is hardcoded as a message
	 *            separator. Sends as ASCII. Include the sending server ID in
	 *            the message
	 ...
	 */
	/*
	 * public synchronized List<String> getReceivedMsgs() { List<String> objs =
	 * new ArrayList<String>(); synchronized(inSockets) {
	 * ListIterator<IncomingSock> iter = inSockets.listIterator(); while
	 * (iter.hasNext()) { IncomingSock curSock = iter.next(); try {
	 * 
	 * } catch (Exception e) { config.logger.log(Level.INFO, "Server " +
	 * config.procNum + " received bad data on a socket", e);
	 * curSock.cleanShutdown(); iter.remove(); } } }
	 * 
	 * return objs; }
	 */
	/**
	...
	public static void main(String[] args) {
		...
		NetController p1_con = new NetController(p1, leader, replica, acceptor,
				commander, scout, heartbeat);
		NetController p2_con = new NetController(p2, leader, replica, acceptor,
				commander, scout, heartbeat);
